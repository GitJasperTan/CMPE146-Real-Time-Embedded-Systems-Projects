...
void spi_id_verification_task(void *p) {
  ...
  const int *receiving_task_num = (int *)(p);

  while (1) {
    if (xSemaphoreTake(mutex, 1000)) {
      adesto_flash_id_s id = {0};
      id = adesto_read_signature();
      fprintf(stderr, "Task#%i: %x\n", *receiving_task_num, id.manufacturer_id);
...
void main(void) {
  ...
  // Used for part 2
  // spi2__init(24);
  // xTaskCreate(spi_id_verification_task, "spi_task", 2048, &task1, PRIORITY_LOW, NULL);
