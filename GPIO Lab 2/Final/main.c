...
void led_task(void *task_parameter) {
  // const port_pin_s *led0 = (port_pin_s *)(task_parameter);

  while (true) {
    // Note: There is no vTaskDelay() here, but we use sleep mechanism while waiting for the binary semaphore (signal)
    if (xSemaphoreTake(switch_press_indication, 1000)) {

      while (!(xSemaphoreTake(switch_press_indication, 1000))) {
        for (int i = 0; i <= 3; i++) {
          gpio0__set_low(led[i].port, led[i].pin);
          //gpio0__set_low(led0->port, led0->pin);
          vTaskDelay(250);
        }
        for (int i = 3; i >= 0; i--) {
          gpio0__set_high(led[i].port, led[i].pin);
          //gpio0__set_high(led0->port, led0->pin);
          vTaskDelay(250);
        }
      }
    } else {
...
int main(void) {
  ...
  static port_pin_s led0 = {1, 26};

  //xTaskCreate(led_task, "led_task", 4096 / sizeof(void *), &led0, PRIORITY_LOW, NULL);
